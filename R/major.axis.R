      ...
      if (nrow(X$PCvalues[[i]]) < min_n) {
        ...
        next} else{
          ...
          for (j in 1:group_resampled$iter){
            ...
            Importance_list[[X$groups[i]]] <- summary(group_major.axis$Pmatrix)
...
posthoc.major.axis.comparison <- function(MASA_result, Comp_MASA_result, group_list, MA_number = 1,  PCs = c(1:4), PC_comp = 1){
  ...
  # ##Save list of normality test of the resampled differences
  # normality_tests <- list()
  # ##
...
##Function to statistically test pairwise angular comparisons between sets of group major axes##
posthoc.major.axis.RRPP <- function(X, Y, Xgroups = NULL,Ygroups = NULL, PCs = c(1:4), PC_comp = 1, MA_number = 1, method = c("bootstrap","jack-knife"), iter = 999, alpha = 0.05, anova_type = "VC"){
  ...
  group_comps <- interaction(Xgroups,Ygroups,sep = ":")
  
  groups <- Z$groups
  resampled_ma <<- resample.major.axis(Z , PCs, MA_number, method, iter, alpha)
